...
public class Lab1 {
	...
	public int run(int mod) {
		...
		Record point1 = recordList.get(0);
		System.out.println("Первая точка " + point1);
		Record point2 = recordList.get(1);
		System.out.println("Вторая точка " + point1);
		Record point3 = recordList.get(2);
		System.out.println("Третья точка " + point1);
		System.out.println("=======Запускаем бесконечный цикл==========");
		while (true) {

			List<Record> k1 = new ArrayList<>();
			List<Record> k2 = new ArrayList<>();
			List<Record> k3 = new ArrayList<>();
